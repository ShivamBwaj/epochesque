...
function slotEnd(i: number) {
  const total = (i + 1) * 15
  return `${String(11 + Math.floor(total / 60)).padStart(2, "0")}:${String(total % 60).padStart(2, "0")}`
}

export function GamingPicker({ games, myBooking }: { games: GameView[]; myBooking: MyBooking | null }) {
            ...
            <Card key={g.game} className="overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-2 p-3 sm:grid-cols-3">
                {g.slots.map((s, i) => {
                  ...
                  const disabled = s.taken || !!myBooking || busySlot !== null || pending
                  return (
                    <button
                      key={s.id}
                      type="button"
